...
export default class Github {
  ...
  async setViewerInformation() {
    response = await this.getViewerInformation()

    this.login = response.data.viewer.login
  }

  getViewerInformation() {
    return this.query('viewer { login }')
  }
  ...
  extractPullRequestInformation(response) {
    ...
    lastCommitStatus  = pullRequest.commits.nodes[0].commit.status
