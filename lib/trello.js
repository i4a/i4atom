...
import Board from './i4atom/models/board'
...
export default class Trello {
  ...
  loadCards(board) {
    board.inProgressListId
        .then(inProgressListId => this.client.makeRequest('get', `/1/lists/${inProgressListId}/cards`, {fields: ['name', 'idMembers', 'url'], actions: 'commentCard'}))
        .then(cards => this.filterUserCards(cards))
        .then(cards => this.setBoard(cards, board))
  ...
  setBoard(cards, board) {
    cards.forEach(card => card.board = board)
