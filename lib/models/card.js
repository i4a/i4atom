...
export default class Card {
  ...
  async openPullRequest () {
    ...
    try {
      pullRequest = await PullRequest.open(this.name, this.url)
    ...
    atom.notifications.addSuccess(`<a href="${pullRequest.url}">${pullRequest.title}</a> opened and added to <a href="${this.url}">card</a>`)
